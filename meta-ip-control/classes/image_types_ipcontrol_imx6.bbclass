...
SDCARD_GENERATION_COMMAND_mx6_ipcontrol = "generate_imx_sdcard_variscite"
#
...
generate_imx_sdcard_variscite () {
	...
	case "${IMAGE_BOOTLOADER}" in
		...
		;;
		u-boot)
		dd if=${DEPLOY_DIR_IMAGE}/u-boot-${MACHINE}.bin of=${SDCARD} conv=notrunc seek=2 skip=${UBOOT_PADDING} bs=512
		;;
		barebox)
		dd if=${DEPLOY_DIR_IMAGE}/barebox-${MACHINE}.bin of=${SDCARD} conv=notrunc seek=1 skip=1 bs=512
		dd if=${DEPLOY_DIR_IMAGE}/bareboxenv-${MACHINE}.bin of=${SDCARD} conv=notrunc seek=1 bs=512k
	...
	# Burn kernel
	dd if=${DEPLOY_DIR_IMAGE}/${KERNEL_IMAGETYPE}-${MACHINE}.bin of=${SDCARD} conv=fsync seek=2048 bs=512
